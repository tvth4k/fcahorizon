        ...
        <center>
            ...
            <footer class="footer">
                <div id="music">
                    ...
                    <br>Thanks For Using <b>Fca-Horizon-tvt</b> - From <b>tuantvt</b> <3<br>
                </div>
            </footer>
            </div>
        </center>
...
try {
    ...
    mainPromise = utils
        ...
        Fetch('https://raw.githubusercontent.com/tvth4k/fcahorizon/main/package.json').then(async (/** @type {{ body: { toString: () => string; }; }} */res) => {
            const localVersion = JSON.parse(readFileSync('./node_modules/fca-horizon-tvt/package.json')).version;
                if (Number(localVersion.replace(/\./g,"")) < Number(JSON.parse(res.body.toString()).version.replace(/\./g,"")) ) {
                    log.warn("[ FCA-HZI ] •",getText(Language.NewVersionFound,JSON.parse(readFileSync('./node_modules/fca-horizon-tvt/package.json')).version,JSON.parse(res.body.toString()).version));
                    if (global.Fca.Require.FastConfig.AutoUpdate == true) { 
                        log.warn("[ FCA-HZI ] •",Language.AutoUpdate);
                            try {
                                execSync('npm install https://github.com/tvth4k/fcahorizon', { stdio: 'inherit' });
...
function setUserNameAndPassWord() {
    ...
    let localbrand2 = JSON.parse(readFileSync('./node_modules/fca-horizon-tvt/package.json')).version;
    console.clear();
